    ...
    if (ESESTATUS_SUCCESS == status) {
      ...
      if (ESESTATUS_SUCCESS != status) {
        ...
        if ((pRspTrans.len > 0) && (pRspTrans.p_data != NULL)) {
          ...
          ALOGD_IF(ese_debug_enabled,"pRspTrans.len error = %d", pRspTrans.len);
          status = ESESTATUS_FAILED;
        }
      }
      if (pCmd_data != NULL) {
...
static ESESTATUS phNxpEse_7816_FrameCmd(pphNxpEse_7816_cpdu_t pCmd,
  ...
  ALOGD_IF(ese_debug_enabled,"%s  pCmd->lc = %d, pCmd->le_type = %d", __FUNCTION__,
  ...
  if (pCmd->lc > 0) {
    ...
    if (pCmd->pdata == NULL) {
      ALOGE("%s Invalide data buffer from application ",
                      __FUNCTION__);
      return ESESTATUS_INVALID_BUFFER;
    }
  } else {
    lc_len = 0;
    ALOGD_IF(ese_debug_enabled,"%s lc (data) field is not present %d", __FUNCTION__,
                    pCmd->lc);
  }
  ...
  if (pCmd->le_type > 0) {
    if (pCmd->le_type == 1) {
      ...
      ALOGE("%s wrong cpdu_type value %d", __FUNCTION__,
                      pCmd->cpdu_type);
  ...
  if (pbuff == NULL) {
    ALOGD_IF(ese_debug_enabled,"%s Error allocating memory", __FUNCTION__);
  ...
  *(pbuff + index) = pCmd->p2;
  ...
  ALOGD_IF(ese_debug_enabled,"Exit %s cmd_total_len = %d, index = %d", __FUNCTION__, index,
                  cmd_total_len);
